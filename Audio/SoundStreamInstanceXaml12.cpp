...
#include "WaveBankReaderXaml12.h"
#include "Graphics/PlatformHelpersXaml12.h"
#include "SoundCommonXaml12.h"
...
HRESULT SoundStreamInstance::Impl::ReadBuffers() noexcept
{
    ...
    for (uint32_t j = 0; j < MAX_BUFFER_COUNT; ++j)
    {
        ...
        if (mPackets[entry].state == State::FREE)
        {
            if (mCurrentPosition < mLengthInBytes)
            {
                auto const cbValid = static_cast<uint32_t>(((mPacketSize) < (mLengthInBytes - mCurrentPosition)) ? (mPacketSize) : (mLengthInBytes - mCurrentPosition));
                // deprecated because there was a failure to cast from size_t to uint32_t
                //std::min(mPacketSize, mLengthInBytes - mCurrentPosition));
