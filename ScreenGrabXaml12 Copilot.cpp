...
#include <shcore.h>
#include <shlwapi.h>
 // Add a hint file to resolve VCR102 error for macros  
 
#include "vcrhint.h"  // Include the hint file for VCR102 error resolution
#include <ppltasks.h> // Include this header to resolve ambiguity for create_task  

using namespace concurrency; // Ensure the correct namespace is used for create_task  
...
namespace
{
    ...
    //--------------------------------------------------------------------------------------
    HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::CaptureTexture(_In_ ID3D12Device* device,
...
HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::SaveDDSTextureToFile(ID3D12CommandQueue* pCommandQueue, ID3D12Resource* pSource, Platform::String^ fileName, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
{
    ...
    *reinterpret_cast<uint32_t*>(&fileHeader[0]) = DDS_MAGIC;
	...
	// Create a FileSavePicker to allow the user to choose where to save the file
    ...
    */
    ...
    create_task(pick->PickSaveFileAsync()).then([this, MAX_HEADER_SIZE, fileHeader, slicePitch, header](StorageFile^ file)
        {
            if (file != nullptr)
            {
                ...
                {
                    HeaderFile->set(i, static_cast<unsigned char>(HeaderFile[j++]));
                ...
                create_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([this, file]()
                    {
                        ...
                        create_task(Windows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([this, file](Windows::Storage::Provider::FileUpdateStatus status)
                            {
        ...
        {
            // Output the file path to debug console
            Platform::String^ str = ref new Platform::String(L"ScreenGrab: Saved texture to ");
            Platform::String^ str1 = str->Concat(str, fileName);
			OutputDebugStringW(str1->Data());

        });



    /* // CoPilot Made Function
	// Use FileSavePicker to save the file
	// Create a FileSavePicker to allow the user to choose where to save the file
    ...
    picker->FileTypeChoices->Insert("Direct Draw Surface", extensions);
    picker->SuggestedFileName = "RotoDrawScreenShot";

    create_task(picker->PickSaveFileAsync()).then([headerBytes, pixelBytes](StorageFile^ file) {
        if (file != nullptr) {
            // Combine header and pixel data into one array
            auto totalSize = headerBytes->Length + pixelBytes->Length;
            auto allBytes = ref new Platform::Array<unsigned char>(totalSize);
            for (unsigned int i = 0; i < headerBytes->Length; ++i)
                allBytes[i] = headerBytes[i];
            for (unsigned int i = 0; i < pixelBytes->Length; ++i)
                allBytes[headerBytes->Length + i] = pixelBytes[i];

            InMemoryRandomAccessStream^ memoryStream = ref new InMemoryRandomAccessStream();
            DataWriter^ writer = ref new DataWriter(memoryStream);
	...
	StorageFile^ file = ref new StorageFile(fileName);
	if (!file)
	{
		return E_INVALIDARG;
	}
	...
	// Create a stream to write the data
	InMemoryRandomAccessStream^ memoryStream = ref new InMemoryRandomAccessStream();
	DataWriter^ dataWriter = ref new DataWriter(memoryStream);
	dataWriter->WriteBytes(HeaderFile);
	...
	// Cleanup
	pStaging.Reset();
	pCommandQueue->Signal(nullptr, 0); // Ensure the command queue is signaled to avoid dangling commands
	if (pStaging)
	{
		pStaging->Unmap(0, nullptr); // Unmap the staging resource
	}
	// Return success
	if (SUCCEEDED(hr))
	{
...
HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::SaveWICTextureToFile(ID3D12CommandQueue* pCommandQ, ID3D12Resource* pSource, REFGUID guidContainerFormat, const wchar_t* fileName, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState, const GUID* targetFormat, std::function<void __cdecl(IPropertyBag2*)> setCustomProps, bool forceSRGB)
{
    if (!fileName)
        return E_INVALIDARG;
    ...
    extensions->Append(".wmp");

    picker->FileTypeChoices->Insert("Direct Draw Surface", extensions);
   // picker->SuggestedFileName = "RotoDrawScreenShot";
	picker->SuggestedFileName = ref new Platform::String(fileName);


    // Replace the ambiguous create_task usage with the fully qualified name
    concurrency::create_task(picker->PickSaveFileAsync()).then([this, pCommandQ, fileName, desc, dstRowPitch, imageSize, pMappedMemory, pfGuid](StorageFile^ file)
        {
            if (file != nullptr)
            {
                ...
                dataWriter->WriteString(ref new Platform::String(fileName));
                // Convert the file name to bytes and write it to the stream
                Platform::Array<unsigned char>^ fileNameBytes = ref new Platform::Array<unsigned char>(static_cast<unsigned int>(wcslen(fileName)) * sizeof(wchar_t));
                for (unsigned int i = 0; i < wcslen(fileName); i++)
                {
                    fileNameBytes->set(i, static_cast<unsigned char>(fileName[i]));
                }
                // Write the file name bytes to the data writer
                dataWriter->WriteBytes(fileNameBytes);

                IBuffer^ buffer1 = dataWriter->DetachBuffer();

                // Example: Writing a texture to a PNG file using WIC
				
                Microsoft::WRL::ComPtr<IWICImagingFactory2> wicFactory = _GetWIC(); // Get your WIC factory
                ComPtr<IWICStream> stream;
               ...
               // wicFactory->CreateEncoder(GUID_ContainerFormatBmp, nullptr, &encoder);

               // wicFactory->CreateEncoder(GUID_ContainerFormatIco, nullptr, &encoder);

               // wicFactory->CreateEncoder(GUID_ContainerFormatTiff, nullptr, &encoder);
//
				//wicFactory->CreateEncoder(GUID_ContainerFormatGif, nullptr, &encoder);
                
				//wicFactory->CreateEncoder(GUID_ContainerFormatWmp, nullptr, &encoder);

				//wicFactory->CreateEncoder(GUID_ContainerFormatRaw, nullptr, &encoder);
                ...
                frame->SetSize(static_cast<UINT>(desc.Width), static_cast<UINT>(desc.Height));
                frame->SetPixelFormat(&WICPixelFormatGUID(pfGuid));

                // Write pixel data (no DDS header involved)
                frame->WritePixels(static_cast<UINT>(desc.Height), static_cast<UINT>(dstRowPitch), static_cast<UINT>(imageSize), static_cast<BYTE*>(pMappedMemory));// pixelData);
                frame->Commit();

                concurrency::create_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([this, file]()
                    {
                        concurrency::create_task(Windows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([this, file](Windows::Storage::Provider::FileUpdateStatus status)
                            {
                                if (status == Windows::Storage::Provider::FileUpdateStatus::Complete)
                                {
                                    Platform::String^ str = ref new Platform::String(L"\nFile ");
    ...
    // Cleanup
    pStaging.Reset();
    pCommandQ->Signal(nullptr, 0); // Ensure the command queue is signaled to avoid dangling commands
    if (pStaging)
    {
        pStaging->Unmap(0, nullptr); // Unmap the staging resource
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        Platform::String^ str = ref new Platform::String(L"ScreenGrab: Saved texture to ");
        Platform::String^ str1 = str->Concat(str, ref new Platform::String(fileName));// ->Path);
        OutputDebugStringW(str1->Data());
    }
    // Return success
    if (hr != S_OK)
...
HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::SaveDDSTextureToFileEx(
    ...
    D3D12_RESOURCE_STATES afterState) noexcept
{
    ...
    ComPtr<ID3D12Resource> pStaging;
    HRESULT hr = DirectX::DXTKXAML12::ScreenGrab::CaptureTexture(device.Get(), pCommandQ, pSource, dstRowPitch, desc, pStaging, beforeState, afterState);
    if (FAILED(hr))
        return hr;
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_
HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::SaveWICTextureToFileEx(ID3D12CommandQueue* pCommandQ, ID3D12Resource* pSource, REFGUID guidContainerFormat, const wchar_t* fileName, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState, const GUID* targetFormat, std::function<void __cdecl(IPropertyBag2*)> setCustomProps, bool forceSRGB)
{
    ...
    D3D12_RESOURCE_STATES afterState)
{
    ...
    *reinterpret_cast<uint32_t*>(&fileHeader[0]) = DDS_MAGIC;
    ...
    ULONG written = 0;
    hr = stream->Write(fileHeader, static_cast<ULONG>(headerSize), &written);
...
HRESULT SaveWICTextureToStream(
    ID3D12CommandQueue* pCommandQ,
    ID3D12Resource* pSource,
    ...
    bool forceSRGB)
{
    ...
    WICPixelFormatGUID pfGuid = {};
    bool sRGB = forceSRGB;
    // (copy the switch/case for pfGuid from your SaveWICTextureToFile implementation)

    // ... (copy the rest of the SaveWICTextureToFile logic, but use the provided IStream* instead of a file)
    ...
    // ... (continue with pixel format, metadata, and pixel writing as in SaveWICTextureToFile)
...
{
    HRESULT __cdecl DirectX::DXTKXAML12::ScreenGrab::SaveDDSTextureToStreamImp(
        ID3D12CommandQueue* pCommandQueue,
        ID3D12Resource* pSource,
        IStream* stream,
        D3D12_RESOURCE_STATES beforeState,
        D3D12_RESOURCE_STATES afterState)
    {
		...
		//  pixels = std::make_unique<BYTE[]>(totalResourceSize); old code, replaced with the new logic below
        // Fix for C4244: 'argument': conversion from 'UINT64' to 'const size_t', possible loss of data  
        pixels = std::make_unique<BYTE[]>(static_cast<size_t>(totalResourceSize));
       ...
       // UINT fpRowCount = 0;
        ...
        if (FAILED(hr))
            return hr;
        // Write to stream
        return SaveDDSTextureToStreamImp(pCommandQueue, pStaging.Get(), stream, beforeState, afterState);
        ...
        D3D12_RESOURCE_STATES afterState)
    {
        ...
        if (FAILED(hr))
            return hr;
        // Write to file
        return SaveDDSTextureToStreamImp(pCommandQueue, pStaging.Get(), stream.Get()/*hFile.get()*/, beforeState, afterState);
    } // namespace DXTKXAML12
    //--------------------------------------------------------------------------------------
    // CreateFromStream
