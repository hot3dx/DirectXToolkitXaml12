...
double Hot3dx::Hot3dx12Rotate::DegreesToRadians(double degree)
{
	m_fCamMove_camerarotation = m_fCamMove_anglerotation * m_fCamMove_degreeradian;
	return m_fCamMove_camerarotation;
}

void Hot3dx::Hot3dx12Rotate::InitSphereVars(void)
{
	//throw ref new Platform::NotImplementedException();
}

float* Hot3dx::Hot3dx12Rotate::CalculateMeshBoxAndCenterCV(CUSTOMVERTEX* v, int count)
{
	static float box[9];
	box[0] = v[0].x;
	box[1] = v[0].x;
...
int Hot3dx::Hot3dx12Rotate::RotateOnMeshZeroAxisCV(CUSTOMVERTEX* v, VOID* pV, int Count, float a, float b, float c)
{
...
int Hot3dx::Hot3dx12Rotate::RotateMeshOnAnyAxisCV(float ox, float oy, float oz, // origin axis
...
void Hot3dx::Hot3dx12Rotate::CalculateSphere(int* count)
{
...
Hot3dx::CUSTOMVERTEX* Hot3dx::Hot3dx12Rotate::CalculateSphereCV(CUSTOMVERTEX* v, int* n, DWORD dwcolor)
{
	// all points on sphere at origin
	// x^2 + y^2 + z^2 = r^2 
	// x^2 + y^2 + z^2 - r^2 = 0
...
Hot3dx::VertexPositionColor3* Hot3dx::Hot3dx12Rotate::CalculateSphereVPC(Hot3dx::VertexPositionColor3* v, int* n, DWORD dwcolor)
{
	...
	if (ox != NULL)
	{
		...
		if (oy != NULL)
		{
			...
			///////////////////////////////////////
			v = (VertexPositionColor3*)malloc((cnt + 2) * sizeof(VertexPositionColor3));
			if (v != NULL)
			{
				m_bArrayInit = true;
				angle = 0.0F;
				...
				for (int i = 0; i < half; i++)
				{
					...
					if (oy[i] != 0.0F)
					{
						...
						for (int j = 0; j < rep; j++)
						{
							v[cnt].pos.x = ((float)ox[i]);// +150.0f;
							v[cnt].pos.y = (float)(radius * (1.0 * cos(zangle)));// +150.0f;
							v[cnt].pos.z = (float)(radius * (1.0 * sin(zangle)));
							dwcolor = 0x090900FF; //Orangish?
							v[cnt].color = DirectX::XMFLOAT3((float)GetRValue(dwcolor) * 0.00390625f, (float)GetGValue(dwcolor) * 0.00390625f, (float)GetBValue(dwcolor) * 0.00390625f);// , (float)GetCValue(dwcolor) * 0.00390625f);
							//v[cnt].color = DirectX::XMFLOAT4((float)GetRValue(dwcolor) * 0.00390625f, (float)GetGValue(dwcolor) * 0.00390625f, (float)GetBValue(dwcolor) * 0.00390625f), (float)GetCValue(dwcolor) * 0.00390625f);
							cnt++;
							zangle += addangle;
						}// eo j for
					}// eo if i < 0
					else
					{
						// First point of the sphere x axis						
						if (cnt == 0) {
							v[cnt].pos.x = (float)ox[0];// +150.0F;
							v[cnt].pos.y = 0.0f;// +150.0F;
							v[cnt].pos.z = 0.0F;
							dwcolor = 0x00FF0000; // Blue
							v[cnt].color = DirectX::XMFLOAT3((float)GetRValue(dwcolor) * 0.00390625f, (float)GetGValue(dwcolor) * 0.00390625f, (float)GetBValue(dwcolor) * 0.00390625f);// , (float)GetCValue(dwcolor) * 0.00390625f);// 0x000000FF; // Blue
				...
				// Last point point of the sphere x axis
				float x = (float)-ox[0];
				v[cnt].pos.x = x;// +150.0F;
				v[cnt].pos.y = 0.0f;// +150.0F;
				v[cnt].pos.z = 0.0F;
				dwcolor = 0x0000FF00; // Green// dwcolor;
				v[cnt].color = DirectX::XMFLOAT3((float)GetRValue(dwcolor) * 0.00390625f, (float)GetGValue(dwcolor) * 0.00390625f, (float)GetBValue(dwcolor) * 0.00390625f);// , (float)GetCValue(dwcolor) * 0.00390625f);// 0x0000FF00; Green// dwcolor;
				//v[cnt].color = XMFLOAT4((float)GetRValue(dwcolor) * 0.00390625f, (float)GetGValue(dwcolor) * 0.00390625f, (float)GetBValue(dwcolor) * 0.00390625f, (float)GetCValue(dwcolor) * 0.00390625f);// 0x000000FF; // Blue
				cnt++;
...
void Hot3dx::Hot3dx12Rotate::MoveRotateCameraXY(int direction)
{
	throw ref new Platform::NotImplementedException();
}

void Hot3dx::Hot3dx12Rotate::MoveRotateCameraAny(float x, float y, float z)
{
	throw ref new Platform::NotImplementedException();
}

float Hot3dx::Hot3dx12Rotate::yCoordofXRot3(float y, float z, float radAngle) {
	return (float)((y * cos(radAngle)) + (z * sin(radAngle)));
}
//////////////////////////.......
float Hot3dx::Hot3dx12Rotate::zCoordofXRot3(float y, float z, float radAngle) {
	return (float)((z * cos(radAngle)) - (y * sin(radAngle)));
}

float Hot3dx::Hot3dx12Rotate::xCoordofYRot3(float x, float z, float a) {
	return (float)((x * cos(a)) + (z * sin(a)));
}

float Hot3dx::Hot3dx12Rotate::zCoordofYRot3(float x, float z, float a) {
	return (float)((z * cos(a)) - (x * sin(a)));
}

float Hot3dx::Hot3dx12Rotate::xCoordofZRot3(float x, float y, float radAngle) {
	//	return (float)((x* cos(a))+(y*(-sin(a))));
	return (float)((x * cos(radAngle)) - (y * sin(radAngle)));
}

float Hot3dx::Hot3dx12Rotate::yCoordofZRot3(float x, float y, float radAngle) {
