...
// portions Copyright (c) Jeff Kubitz - hot3dx
//
...
public:
    ...
    void _cdecl SetGPUHandle(_In_ HANDLE handle) { mCompletedEventImpl = handle; }
    HANDLE _cdecl GetGPUHandle() { return mCompletedEventImpl; }
    ////////////////////////
    ///////// EndXaml()
...
Concurrency::task<void> EndXaml(
        _In_ ID3D12CommandQueue* commandQueue)
    {
        ...
        // Submit the job to the GPU
        ID3D12CommandList* ppCommandLists[] = { mList.Get() };
        ...
        //auto createUploadBatch = create_async([this, uploadBatch] {
            auto doBatch = create_task([this, uploadBatch]()
            {
                ...
                SetGPUHandle(uploadBatch->GpuCompleteEvent);
                // Delete the batch
                // Because the vectors contain smart-pointers, their destructors will
                // fire and the resources will be released.
                delete uploadBatch;
