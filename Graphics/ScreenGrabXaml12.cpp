...
namespace
{
        ...
        D3D12_RESOURCE_STATES afterState)
    {
        ...
        {
            DirectX::DXTKXAML12::DebugTrace("ERROR: ScreenGrab does not support 1D or volume textures. Consider using DirectXTex instead.\n");
        ...
        bufferFootprint.Footprint.Height = static_cast<UINT>(desc.Height);
...
namespace DirectX {
    namespace DXTKXAML12 {
        ...
        /// <summary>
        /// GetBytesPerPixel(DXGI_FORMAT format)
        /// Used to get the number of Bytes per Pixels
        /// </summary>
        ...
        HRESULT __cdecl ScreenGrab::SaveDDSTextureToFile(ID3D12CommandQueue* pCommandQueue, ID3D12Resource* pSource, Windows::Storage::StorageFile^ file, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
        {
            ...
            *reinterpret_cast<uint32_t*>(&fileHeader[0]) = DDS_MAGIC;
            ...
            SetDebugObjectName(fence.Get(), L"ScreenGrab2");
            ...
            DWORD waitResult = WaitForSingleObject(fenceEvent, 5000); // 5 seconds
            CloseHandle(fenceEvent);

            if (waitResult != WAIT_OBJECT_0) {
                // Timeout or error
                return DXGI_ERROR_DEVICE_HUNG; // Or another appropriate error code
            }

            // Now it is safe to map the resource for CPU readback

            void* pMappedMemory = nullptr;
            D3D12_RANGE readRange = { 0, static_cast<SIZE_T>(totalResourceSize) };
            D3D12_RANGE writeRange = { 0, 0 };
            hr = pStaging->Map(0, &readRange, &pMappedMemory);
            if (FAILED(hr)){
                char buf[128];
                sprintf_s(buf, "Map failed: 0x%08X\n", hr);
                OutputDebugStringA(buf);

                HRESULT deviceRemovedReason = device->GetDeviceRemovedReason();
                char buf1[128];
                sprintf_s(buf, "Device removed reason: 0x%08X\n", deviceRemovedReason);
                OutputDebugStringA(buf1);
                // Optionally, handle device lost here
            ...
            unsigned int sz2 = (unsigned int)slicePitch;
            Platform::Array<uint8_t>^ HeaderFile1 = ref new Platform::Array<uint8_t>(sz2);// ref new Platform::Array<unsigned char>(sz2);

            uint8_t* pptr = pixels.get();
            for (unsigned int i = 0; i < sz2; i++)
            {
                HeaderFile1[i] = pptr[i];
            }
           
            if (file != nullptr)
            {
                unsigned int sz1 = (unsigned int)MAX_HEADER_SIZE;
                unsigned int total = sz1 + (unsigned int)slicePitch;
                Platform::Array<unsigned char>^ HeaderFile = ref new Platform::Array<unsigned char>(total);
                for (unsigned int i = 0; i < sz1; i++)
                {
                    HeaderFile->set(i, static_cast<unsigned char>(fileHeader[i]));

                }

                unsigned int j = 0;
                for (unsigned int i = sz1; i < total; i++)
                {
                    HeaderFile->set(i, static_cast<unsigned char>(HeaderFile1[j++]));
                ...
                dataWriter->WriteBytes(HeaderFile);
                IBuffer^ buffer1 = dataWriter->DetachBuffer();



                create_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([this, file]()
                    {

                        create_task(Windows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([file](Windows::Storage::Provider::FileUpdateStatus status)
                            {
                                if (status == Windows::Storage::Provider::FileUpdateStatus::Complete)
                                {
                                    OutputDebugStringW(L"SaveWICToTextureFile()  was saved.\n");
                                }
                                else
                                {
                                    if (file != nullptr)
                                    {
                                        ...
                                        create_task(file->DeleteAsync()).then([file](task<void> t)
                                            {
                                                try { t.get(); }
                                                catch (...) { /* Ignore if file didn't exist */ }
                                                // Now proceed to save
                                                // ... your save logic here ...
                                            });
                                    }
                                    OutputDebugStringW(L"SaveWICToTextureFile()  was not saved.\n");
                                }
                        ...
                        OutputDebugStringW(L"\n\nWindows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([file](Windows::Storage::Provider::FileUpdateStatus status)\n");
                       
                        OutputDebugStringW(L"\n\nWindows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([file](Windows::Storage::Provider::FileUpdateStatus status)\n");
                        
                 }).then([this](task<void> previousTask) {

                     OutputDebugStringW(L"\n\nOcreate_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([this, file]()\n");
       ...
       return S_OK;
            ...
            /// <summary>
            /// The SaveWICTextureToFile(...) Uses an InMemoryRandomAccess stream Async task
            /// to write the Memory of a ScreenGrab.
            /// </summary>
            /// <param name="pCommandQ"></param>
            ...
            /// <param name="setCustomProps"></param>
            /// <param name="forceSRGB"></param>
            /// <returns></returns>
            HRESULT __cdecl ScreenGrab::SaveWICTextureToFile(
                ...
                bool forceSRGB)
            {
                ...
                if (!file)
                {
                    OutputDebugStringW(L" \n NO file ScreenGrab::SaveWICTextureToFile(\n");
                    return E_INVALIDARG;
                }
                ...
                // Signal the fence
                hr = pCommandQ->Signal(fence.Get(), fenceValue);
                if (FAILED(hr)) {
                    CloseHandle(fenceEvent);
                    return hr;
                }
                ...
                // Now it is safe to map the resource for CPU readback
                // 
                // ... (format selection code unchanged) ...
                // 2. Map the staging resource
                void* pMappedMemory = nullptr;
                D3D12_RANGE readRange = { 0, static_cast<SIZE_T>(totalResourceSize) };
                D3D12_RANGE writeRange = {0,0};
                hr = pStaging->Map(0, &readRange, &pMappedMemory);
                ...
                // 1. Create an in-memory WIC stream
                ComPtr<IWICStream> wicStream;
                hr = pWIC->CreateStream(wicStream.GetAddressOf());
                if (FAILED(hr))
                    return hr;

                hr = wicStream->InitializeFromMemory(static_cast<uint8_t*>(pMappedMemory), (DWORD)totalResourceSize); // Start with empty memory and there is a loud croaking, gasping sound as it fails
                if (FAILED(hr)) 
                    return hr;
               
                // 2. Encode to the in-memory stream as usual
                ComPtr<IWICBitmapEncoder> encoder;
                hr = pWIC->CreateEncoder(guidContainerFormat, targetFormat, encoder.GetAddressOf());
                if (FAILED(hr))
                    return hr;
                ...
                if (memcmp(&targetGuid, &pfGuid, sizeof(WICPixelFormatGUID)) != 0)
                {
                    ...
                    IStream* pIStream = wicStream.Get();
                    STATSTG statstg = {};
                    hr = pIStream->Stat(&statstg, STATFLAG_NONAME);
                    if (FAILED(hr))
                        return hr;

                    ULONG cbSize = static_cast<ULONG>(statstg.cbSize.QuadPart);
                    std::unique_ptr<BYTE[]> buffer(new (std::nothrow) BYTE[cbSize]);
                    if (!buffer)
                        return E_OUTOFMEMORY;

                    LARGE_INTEGER zero = {};
                    hr = pIStream->Seek(zero, STREAM_SEEK_SET, nullptr);
                    if (FAILED(hr))
                        return hr;

                    ULONG cbRead = 0;
                    hr = pIStream->Read(buffer.get(), cbSize, &cbRead);
                    if (FAILED(hr) || cbRead != cbSize)
                        return E_FAIL;
                   
                    size_t bytesPerPixel = 4;// GetBytesPerPixel(desc.Format);
                                 
                              
                    size_t rowSize = static_cast<size_t>(desc.Width * bytesPerPixel);
                std::vector<BYTE> contiguousBuffer(rowSize* desc.Height);

                BYTE* src = static_cast<BYTE*>(pMappedMemory);
                BYTE* dst = contiguousBuffer.data();

                for (UINT y = 0; y < desc.Height; ++y) {
                    memcpy(dst, src, rowSize);      // Copy only the valid row data
                    src += dstRowPitch;             // Advance by the GPU row pitch
                    dst += rowSize;                 // Advance by the image row size
                }
#if defined(_DEBUG)
                /*
                    auto sptr = static_cast<const uint8_t*>(pMappedMemory);
                    for (size_t i = 0; i < 1000; ++i) {
                        char buf[64];
                        sprintf_s(buf, "Pixel[%zu]: %02X\n", i, sptr[i]);
                        OutputDebugStringA(buf);
                    }
                    */
#endif
                    // After WIC encoding (frame->WriteSource, frame->Commit, encoder->Commit):

// 1. Get the encoded image data from the IWICStream (which is an IStream)
                    STATSTG stat = {};
                    hr = wicStream->Stat(&stat, STATFLAG_NONAME);
                    if (FAILED(hr)) return hr;

                    
                    wicStream->Seek(zero, STREAM_SEEK_SET, nullptr);
                    wicStream->Read(buffer.get(), cbSize, &cbRead);
                    if (cbRead != cbSize) return E_FAIL;

                    // 2. Create an InMemoryRandomAccessStream and copy the bytes
                    InMemoryRandomAccessStream^ memoryStream = ref new InMemoryRandomAccessStream();
                    DataWriter^ dataWriter = ref new DataWriter(memoryStream);
                    dataWriter->WriteBytes(ref new Platform::Array<unsigned char>(buffer.get(), cbSize));
                    IBuffer^ buffer1 = dataWriter->DetachBuffer();
                    ...
                    // 3. Write the buffer to the file asynchronously
                    //concurrency::create_task(Windows::Storage::FileIO::WriteBufferAsync(file, fileBuffer)).wait();
                ...
                else
                {
                    ...
                    // 3. Extract the buffer from the WIC stream
...
// 1. Get the encoded image data from the IWICStream (which is an IStream)
                    ...
                    //InMemoryRandomAccessStream^ memoryStream = ref new InMemoryRandomAccessStream();
                    //DataWriter^ dataWriter = ref new DataWriter(memoryStream);
                    //dataWriter->WriteBytes(dataArray);
                    //IBuffer^ buffer1 = dataWriter->DetachBuffer();
                    //concurrency::create_task(Windows::Storage::FileIO::WriteBytesAsync(file, dataArray)).wait();
                    create_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([this, file]()
                        {
            ...
            HRESULT __cdecl ScreenGrab::SaveTextureToPngFile(ID3D12CommandQueue* pCommandQ, ID3D12Resource* pSource, Windows::Storage::StorageFile^ file, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
            {
                    // 1. Capture to staging resource (readback heap)
                    // ... (your CaptureTexture code) ...
                if (!file)
                    return E_INVALIDARG;
                ...
                if (FAILED(hr))
                    ...
                    // 3. Create WIC factory and in-memory stream
                    ComPtr<IWICImagingFactory2> pWIC = _GetWIC();
                    ComPtr<IWICStream> wicStream;
                    hr = pWIC->CreateStream(&wicStream);
                    if (FAILED(hr)) return hr;
                    hr = wicStream->InitializeFromMemory(static_cast<BYTE*>(pMappedMemory), 0); // in-memory
                    if (FAILED(hr)) return hr;

                    // 4. Create encoder and frame
                    ComPtr<IWICBitmapEncoder> encoder;
                    hr = pWIC->CreateEncoder(GUID_ContainerFormatPng, nullptr, &encoder);
                    if (FAILED(hr)) return hr;
                    hr = encoder->Initialize(wicStream.Get(), WICBitmapEncoderNoCache);
                    if (FAILED(hr)) return hr;
                    ...
                    hr = frame->Initialize(props.Get());
                    if (FAILED(hr)) return hr;
                    hr = frame->SetSize((UINT)desc.Width, (UINT)desc.Height);
                    if (FAILED(hr)) return hr;

                    GUID WICPixelFormatGUID = GUID_WICPixelFormat32bppBGRA;
                    hr = frame->SetPixelFormat(&WICPixelFormatGUID); // or your format
                    if (FAILED(hr)) return hr;

                    // 5. Write pixels
                    hr = frame->WritePixels(desc.Height, (UINT)fpRowPitch, (UINT)totalResourceSize, static_cast<BYTE*>(pMappedMemory));
                    if (FAILED(hr)) return hr;
                    ...
                    // 8. Extract buffer and write to file
                    STATSTG stat = {};
                    hr = wicStream->Stat(&stat, STATFLAG_NONAME);
                    if (FAILED(hr)) return hr;
                    ULONG cbSize = static_cast<ULONG>(stat.cbSize.QuadPart);
                    std::unique_ptr<BYTE[]> buffer(new BYTE[cbSize]);
                    LARGE_INTEGER zero = {};
                    wicStream->Seek(zero, STREAM_SEEK_SET, nullptr);
                    ULONG cbRead = 0;
                    wicStream->Read(buffer.get(), cbSize, &cbRead);

                    auto dataArray = ref new Platform::Array<unsigned char>(cbSize);
                    memcpy(dataArray->Data, buffer.get(), cbSize);
                            ...
                            create_task(Windows::Storage::CachedFileManager::CompleteUpdatesAsync(file)).then([file](Windows::Storage::Provider::FileUpdateStatus status)
                                {
                                    if (status == Windows::Storage::Provider::FileUpdateStatus::Complete)
                                    {
                                        Platform::String^ str = ref new Platform::String(L"\nFile ");
                                        Platform::String^ str1 = str->Concat(str, file->Path);
                                        Platform::String^ str2 = str->Concat(str1, ref new Platform::String(L" was saved.\n"));
                                        OutputDebugStringW(str2->Data());
                                    }
                                    else
                                    {
                                        Platform::String^ str = ref new Platform::String(L"\nFile ");
                                        Platform::String^ str1 = str->Concat(str, file->Path);
                                        Platform::String^ str2 = str->Concat(str1, ref new Platform::String(L" couldn't be saved.\n"));
                                        OutputDebugStringW(str2->Data());
                                    }
                                }).then([](task<void> previousTask) {
    ...
    } // eo namespace DXTKXAML12
} // eo namespace DirectX
...
namespace DirectX
{
            ...
            D3D12_RESOURCE_STATES afterState) noexcept
    {
        ...
        if (!hFile)
        {
            ...
            wchar_t buf[256];
            swprintf_s(buf, L"CreateFile2 failed for %s, error %lu\n", fileName, err);
...
namespace DirectX
{
    namespace DXTKXAML12
    {
        ...
        HRESULT __cdecl ScreenGrab::SaveDDSTextureToFileIMemoryRandomAccess(ID3D12CommandQueue* pCommandQueue, ID3D12Resource* pSource, Windows::Storage::StorageFile^ file, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
        {
        ...
        /// <summary>
        /// Saves a Direct3D 12 texture resource to a DDS file using a random access file interface.
        /// </summary>
        /// <param name="pCommandQueue">Pointer to the ID3D12CommandQueue used for resource operations.</param>
        /// <param name="pSource">Pointer to the ID3D12Resource representing the source texture to save.</param>
        /// <param name="fileName">The name of the file to save the DDS texture to. Must not be null.</param>
        /// <param name="beforeState">The resource state of the texture before the copy operation.</param>
        /// <param name="afterState">The resource state of the texture after the copy operation.</param>
        /// <returns>Returns an HRESULT indicating success or failure. Returns E_INVALIDARG for invalid arguments, HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW) for size overflows, or other error codes on failure.</returns>
        
        HRESULT __cdecl ScreenGrab::SaveDDSTextureToFileIRandomAccess(ID3D12CommandQueue* pCommandQueue, ID3D12Resource* pSource, Platform::String^ fileName, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState)
        {
            ...
            *reinterpret_cast<uint32_t*>(&fileHeader[0]) = DDS_MAGIC;
            ...
            pCommandQueue->GetDevice(IID_GRAPHICS_PPV_ARGS(device12.GetAddressOf()));
            ...
            ScreenGrab::CreateFromStream(device12.Get(), commandList.Get(), stream.Get(), &pSource, (D3D12_SUBRESOURCE_DATA*)nullptr, beforeState, false);
            ...
            plainTextExtensions->Append(".dds");
            Platform::String^ type = ref new Platform::String(L"Direct Draw Surface");
            pick->FileTypeChoices->Insert(type, plainTextExtensions);
            pick->SuggestedFileName = ref new Platform::String(L"RotoDrawScreenShot");

            //Platform::String^ s = ref new Platform::String(L"Assets\\Textures\\");
            /*
            ...
            sfile = sfile->Concat(sfile, fileName);
            */
            
            create_task(pick->PickSaveFileAsync()).then([MAX_HEADER_SIZE, fileHeader, slicePitch, HeaderFile1, stream, pStaging](StorageFile^ file)
                {
                    if (file != nullptr)
                    {
                        unsigned int sz1 = (unsigned int)MAX_HEADER_SIZE;
                        ...
                        unsigned int headerSize = sz1;
                       
                       

                        
                        // Write header & pixels to stream
                        ULONG written = 0;
                        HRESULT hr = stream->Write(fileHeader, static_cast<ULONG>(headerSize), &written);
                        if (FAILED(hr) || written != headerSize)
                            return E_FAIL;

                        hr = stream->Write(pStaging.Get(), static_cast<ULONG>(slicePitch), &written);// pixels.get(), static_cast<ULONG>(slicePitch), &written);
                        if (FAILED(hr) || written != slicePitch)
                            return E_FAIL;

                        /*

                        create_task(Windows::Storage::FileIO::WriteBufferAsync(file, buffer1)).then([file]()
                            {
...
*/
        ...
        /// <summary>
        /// Saves a Direct3D 12 texture resource to a file using the Windows Imaging Component (WIC) encoder, with support for custom properties and format conversion.
        /// </summary>
        /// <param name="pCommandQ">Pointer to the ID3D12CommandQueue used for resource operations.</param>
        /// <param name="pSource">Pointer to the ID3D12Resource representing the source texture to save.</param>
        /// <param name="guidContainerFormat">GUID specifying the desired WIC container format (e.g., PNG, JPEG).</param>
        /// <param name="fileName">Wide-character string specifying the output file path.</param>
        /// <param name="beforeState">The resource state of the source texture before the operation.</param>
        /// <param name="afterState">The resource state of the source texture after the operation.</param>
        /// <param name="targetFormat">Pointer to a GUID specifying the desired WIC pixel format for the output file, or nullptr to use a default format.</param>
        /// <param name="setCustomProps">A std::function callback to set custom properties on the WIC property bag before encoding.</param>
        /// <param name="forceSRGB">Boolean flag indicating whether to force sRGB format for the output image.</param>
        /// <returns>Returns an HRESULT indicating success or failure. Returns S_OK on success, or an error code (such as E_INVALIDARG, E_NOINTERFACE, HRESULT_FROM_WIN32(ERROR_NOT_SUPPORTED), or HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW)) on failure.</returns>
        HRESULT __cdecl ScreenGrab::SaveWICTextureToFileIMemoryRandomAccess(ID3D12CommandQueue* pCommandQ, ID3D12Resource* pSource, REFGUID guidContainerFormat, Windows::Storage::StorageFile^ file, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState, const GUID* targetFormat, std::function<void __cdecl(IPropertyBag2*)> setCustomProps, bool forceSRGB)
        {
            if (!file)
                return E_INVALIDARG;
            ...
            if (FAILED(hr))
                return hr;
            /*
            // After issuing copy commands
            pCommandQ->Signal(fence.Get(), fenceValue);
            ...
            */
            ...
            hr = stream->InitializeFromMemory(static_cast<BYTE*>(pMappedMemory), (DWORD)totalResourceSize);
           // hr = stream->InitializeFromFilename(fileName, GENERIC_WRITE);
            if (FAILED(hr))
                return hr;
        ...
        /// <summary>
        /// ////////////////////////////////////////////////////////////////////////
        ///  SaveWICTextureToFileIRandomAccess(
        /// <param name="pCommandQ"></param>
        /// <param name="pSource"></param>
        /// <param name="guidContainerFormat"></param>
        ...
        /// </summary>
        HRESULT __cdecl ScreenGrab::SaveWICTextureToFileIRandomAccess(ID3D12CommandQueue* pCommandQ, ID3D12Resource* pSource, REFGUID guidContainerFormat, Platform::String^ fileName, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState, const GUID* targetFormat, std::function<void __cdecl(IPropertyBag2*)> setCustomProps, bool forceSRGB)
        {
            if (!fileName)
                return E_INVALIDARG;
            ...
            auto_delete_file_wic delonfail(stream, fileName->Data());
        ...
        /// <summary>
        /// Creates a  WIC  Direct3D 12 texture resource from a data stream.
        /// </summary>
        /// <param name="device">Pointer to the ID3D12Device used to create the texture resource.</param>
        /// <param name="commandList">Pointer to the ID3D12GraphicsCommandList used for resource initialization commands.</param>
        /// <param name="stream">Pointer to the IStream containing the image data.</param>
        /// <param name="texture">Address of a pointer that will receive the created ID3D12Resource texture.</param>
        /// <param name="subresource">Pointer to a D3D12_SUBRESOURCE_DATA structure that will be filled with subresource data for the texture.</param>
        /// <param name="initialState">The initial resource state for the created texture.</param>
        /// <param name="forceSRGB">If true, forces the texture format to sRGB.</param>
        /// <returns>Returns an HRESULT indicating success or failure of the operation.</returns>
        HRESULT __cdecl ScreenGrab::CreateFromStream(ID3D12Device* device, ID3D12GraphicsCommandList* commandList, IStream* stream, ID3D12Resource** texture, D3D12_SUBRESOURCE_DATA* subresource, D3D12_RESOURCE_STATES initialState, bool forceSRGB)
        {
            return E_NOTIMPL;
        }
    } // eo DXTKXAML12
